                ...
                using (var workbook = new XLWorkbook(filePath))
                {
                    ...
                    using (var connection = new SqliteConnection(_connectionString))
                    {
                        ...
                        using (var transaction = connection.BeginTransaction())
                        {
                            ...
                            int count = 0;
                            foreach (var groupName in ScheduleConfig.AvailableGroups)
                            {
                            ...
                            transaction.Commit();
                            _logger?.Log($"✅ База обновлена! Групп найдено: {ScheduleConfig.AvailableGroups.Length}, Занятий загружено: {count}");
                        }
                    }
                }
            }
